...
/**
 * Utility functions for file format detection, shared by all storage backends.
 *
 * Previously lived as the companion object of `StorageService`.
 */
object StorageUtils {
     ...
     */
    fun detectFileFormat(bytes: ByteArray): String? = when {
        bytes.isEmpty() -> null
        bytes.hasPrefix(0x25, 0x50, 0x44, 0x46) -> "application/pdf"
        // Legacy .office files
        bytes.hasPrefix(0xD0, 0xCF, 0x11, 0xE0) -> "application/vnd.ms-office"
        // modern office files
        // word
        bytes.hasPrefix(0x50, 0x4B, 0x03, 0x04) && bytes.isDocxPackage() ->
            "application/vnd.openxmlformats-officedocument.wordprocessingml.document"
        // powerpoint
        bytes.hasPrefix(0x50, 0x4B, 0x03, 0x04) &&
        ...
        bytes.hasPrefix(0x49, 0x44, 0x33) || bytes.hasPrefix(0xFF, 0xFB) -> "audio/mpeg"
        ...
        bytes.hasPrefix(0x37, 0x7A, 0xBC, 0xAF, 0x27, 0x1C) -> "application/x-7z-compressed"
        bytes.hasPrefix(0x52, 0x61, 0x72, 0x21, 0x1A, 0x07, 0x00) ||
            bytes.hasPrefix(0x52, 0x61, 0x72, 0x21, 0x1A, 0x07, 0x01, 0x00) -> "application/vnd.rar"
        bytes.hasPrefix(0x49, 0x49, 0x2A, 0x00) || bytes.hasPrefix(0x4D, 0x4D, 0x00, 0x2A) -> "image/tiff"
        bytes.hasIsoBmffBrand("heic", "heif", "hevc", "mif1", "msf1") -> "image/heic"
        bytes.startsWithAscii("<svg") -> "image/svg+xml"
        bytes.hasIsoBmffBrand("isom", "iso2", "mp41", "mp42", "avc1", "dash") -> "video/mp4"
        ...
        bytes.hasPrefix(0xD0, 0xCF, 0x11, 0xE0) && bytes.containsAscii("PowerPoint Document") ->
    ...
    private fun ByteArray.hasPrefix(vararg signature: Int): Boolean {
        ...
        signature.forEachIndexed { index, value -> if (this[index] != value.toByte()) return false }
        return true
    }

    private fun ByteArray.hasRiffType(expected: String): Boolean {
        if (size < 12 || expected.length != 4) return false
        return hasPrefix(0x52, 0x49, 0x46, 0x46) && copyOfRange(8, 12).contentEquals(expected.toByteArray())
    }

    private fun ByteArray.hasIsoBmffBrand(vararg brands: String): Boolean {
        if (size < 12) return false
        if (!copyOfRange(4, 8).contentEquals(byteArrayOf(0x66, 0x74, 0x79, 0x70))) return false
    ...
    private fun ByteArray.hasTarMagic(): Boolean =
        size >= 262 && copyOfRange(257, 262).contentEquals("ustar".toByteArray())

    private fun ByteArray.startsWithAscii(prefix: String): Boolean {
        if (isEmpty()) return false
        var offset = if (size >= 3 && this[0] == 0xEF.toByte() && this[1] == 0xBB.toByte() && this[2] == 0xBF.toByte()) 3 else 0
        while (offset < size && this[offset].toInt().toChar().isWhitespace()) offset++
        if (size - offset < prefix.length) return false
        val head = copyOfRange(offset, offset + prefix.length).decodeToString().lowercase()
    ...
    private fun ByteArray.isOpcPackageWithEntry(requiredEntry: String): Boolean = try {
        ZipInputStream(ByteArrayInputStream(this)).use { zip ->
            var entry = zip.nextEntry
            while (entry != null) {
                if (entry.name.equals(requiredEntry, ignoreCase = true)) return true
                entry = zip.nextEntry
            }
            false
        }
